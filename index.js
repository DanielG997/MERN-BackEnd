...
app.put("/update-quantity", async (req, res) => {
    const nuevaCantComida = req.body.nuevaCantComida;
    const id = req.body.id;

    try {
        await FoodModel.findById(id, (err, cantComidActualizada) => {
            cantComidActualizada.cantidadComida = nuevaCantComida
            cantComidActualizada.save();
            res.send("Actualizado")
        })
    }catch(err) {
        console.log(err);
